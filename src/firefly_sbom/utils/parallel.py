        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    logger.info(f"[{completed}/{total}] Completed scanning: {repo_name}")
                    
                except Exception as e:
                    logger.error(f"[{completed}/{total}] Failed to scan {repo_name}: {e}")
    ...
    def _scan_single_repo(
        ...
        with tempfile.TemporaryDirectory(prefix=f"sbom_{repo_name}_") as tmpdir:
            ...
            try:
                ...
                sbom_data['metadata']['primary_language'] = repo.get('language', 'Unknown')
    ...
    def process_in_batches(
        ...
        **kwargs
        ...
        for i in range(0, len(items), self.batch_size):
            ...
            logger.info(f"Processing batch {i//self.batch_size + 1} of {len(items)//self.batch_size + 1}")
