...
class SBOMGenerator:
    ...
    def scan_repository(
        ...
        for tech in tech_stack:
            ...
            if scanner_type in self.scanners:
                ...
                try:
                    ...
                    logger.info(f"Found {len(components)} components for {tech['name']}")
        ...
        sbom_data = {
            ...
            'stats': {
                ...
                'direct_deps': sum(1 for c in unique_components if c.get('scope') == 'direct'),
                'transitive_deps': sum(1 for c in unique_components if c.get('scope') == 'transitive'),
            }
        }
    ...
    def _generate_repo_report(
        ...
        try:
            ...
            logger.debug(f"Generated {format_type} report for {repo_name}: {output_path}")
            
        except Exception as e:
            logger.error(f"Failed to generate {format_type} report for {repo_name}: {e}")
    
    def _create_org_summary(self, org: str, scan_results: Dict[str, Dict]) -> Dict[str, Any]:
        ...
        for repo_name, sbom_data in scan_results.items():
            if sbom_data and 'components' in sbom_data:
                ...
                repo_summaries.append({
                    ...
                    'technologies': sbom_data.get('metadata', {}).get('technologies', []),
                    'status': 'success'
                })
            else:
                repo_summaries.append({
        ...
        return {
            ...
            'successful_scans': sum(1 for r in repo_summaries if r['status'] == 'success'),
            ...
            try:
                if format_type == 'json':
                    ...
                    if 'html' in self.generators:
                        self.generators['html'].generate_org_summary(org_summary, output_path)
