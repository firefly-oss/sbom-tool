...
class AuditConfig:
    ...
    vulnerability_databases: List[str] = field(default_factory=lambda: ['nvd', 'osv', 'ghsa'])
...
class Config:
    ...
    def __init__(self, config_dict: Optional[Dict[str, Any]] = None):
        ...
        # Additional settings
        self.github_token = config_dict.get('github', {}).get('token', os.getenv('GITHUB_TOKEN'))
    ...
    def from_env(cls) -> 'Config':
        ...
        if os.getenv('SBOM_MAX_DEPTH'):
            config_dict.setdefault('scan', {})['max_depth'] = int(os.getenv('SBOM_MAX_DEPTH'))
        ...
        if os.getenv('SBOM_SEVERITY_THRESHOLD'):
            config_dict.setdefault('audit', {})['severity_threshold'] = \
                os.getenv('SBOM_SEVERITY_THRESHOLD')
        
        # Output configuration from env
        if os.getenv('SBOM_OUTPUT_FORMATS'):
            config_dict.setdefault('output', {})['formats'] = \
                os.getenv('SBOM_OUTPUT_FORMATS').split(',')
    ...
    def validate(self) -> List[str]:
        ...
        if self.audit.severity_threshold not in valid_severities:
            warnings.append(f"Invalid severity threshold: {self.audit.severity_threshold}")
        
        # Check output formats
        valid_formats = ['cyclonedx-json', 'cyclonedx-xml', 'spdx-json', 'spdx-yaml', 'html']
        ...
        if self.cache.enabled and not cache_dir.parent.exists():
            warnings.append(f"Cache directory parent does not exist: {cache_dir.parent}")
